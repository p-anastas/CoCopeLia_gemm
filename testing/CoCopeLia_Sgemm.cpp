...
int main(const int argc, const char *argv[]) {
	...
	if (strcmp(MACHINE, machine)) error("CoCopeLia_Sgemm: Running on wrong machine");
	...
	if (Tin > 0) {
		...
		sprintf(filename, "%s/Data_manipulation/Results/%s/validation/CoCopeLia_Sgemm_%d_v%s.log", PROJECTDIR, MACHINE, dev_id, VERSION);
		check_log(filename, M, N, K, Tin, A_loc, B_loc, C_loc, dev_id);
	}
	else {
		sprintf(filename, "%s/Data_manipulation/Results/%s/evaluation/CoCopeLia_Sgemm_%d_v%s.log", PROJECTDIR, MACHINE, dev_id, VERSION);
		// TODO: Preliminary predict tile in order to avoid repeating benchmarks. NOT required.
		CoCoModel_p model = CoCoModel_gemm_init( M, N, K, A_loc, B_loc, C_loc, dev_id, "Sgemm", 1);
	...
	cpu_timer = csecond();
	CoCopeLia_Sgemm(cpu_op_A, cpu_op_B, M, N, K, alpha, A, ldA, B, ldB, beta, C , ldC, &Tin, dev_id);
	cudaCheckErrors();
	cpu_timer  = csecond() - cpu_timer;
	fprintf(stderr,"First CoCopelia SGEMM call-> M = %zu, N = %zu, K = %zu\n", M, N, K);
	...
	fprintf(stderr,"Running CoCopeLia SGEMM-> M = %zu, N = %zu, K = %zu\n", M, N, K);
	...
	cublasXt_sgemm_wrapper(gpu_op_A,  gpu_op_B, M, N, K, alpha, A, ldA, B, ldB, beta, C, ldC,  (size_t) fmin(fmin(M,N),K)/2/2, 0, dev_id);
 	if (C_loc) optimized_memcpy(C_out1, C,  M * N *sizeof(float));
	else vec_get_memcpy(C_out1, C,  M * N *sizeof(float), dev_id);
