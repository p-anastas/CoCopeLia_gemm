...
int main(const int argc, const char *argv[]) {
	...
	if (Tin > 0){
		...
		sprintf(filename, "%s/Data_manipulation/Results/%s/validation/cuBLASXt_Dgemm_%d.log", PROJECTDIR, MACHINE, dev_id);	
		check_log(filename, M, N, K, Tin, A_loc, B_loc, C_loc, dev_id);
	}
	else {
		sprintf(filename, "%s/Data_manipulation/Results/%s/evaluation/cuBLASXt_Dgemm_%d.log", PROJECTDIR, MACHINE, dev_id);
	...
	cpu_timer = csecond();
	cublasXt_dgemm_wrapper(gpu_op_A,  gpu_op_B, M, N, K, alpha, A, ldA, B, ldB, beta, C, ldC, cublasXt_tile, 0, dev_id);
	cudaCheckErrors();
	cpu_timer  = csecond() - cpu_timer;
	...
	if (Tin > 0){
		...
		for (size_t T_trial = 1024; T_trial < 20000; T_trial+=1024) if (M >= T_trial*1.5 && N >= T_trial*1.5 && K >= T_trial*1.5){
			fprintf(stderr,"Running CUBLASXT DGEMM-> M = %zu, N = %zu, K = %zu, T = %zu\n", M, N, K, T_trial);
			cpu_timer  = csecond();
			cublasXt_dgemm_wrapper(gpu_op_A,  gpu_op_B, M, N, K, alpha, A, ldA, B, ldB, beta, C, ldC, T_trial, 0, dev_id);
			cudaCheckErrors();
			cpu_timer  = csecond() - cpu_timer;
			fprintf(stderr, "Total time:\t%lf ms\n", cpu_timer  * 1000);
