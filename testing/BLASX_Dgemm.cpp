...
int main(const int argc, const char *argv[]) {
	...
	if (strcmp(MACHINE, machine)) error("cuBLASXt_Dgemm: Running on wrong machine");
	...
	if (Tin > 0) {
		if (Tin != BLOCKSIZE_DGEMM) warning("BLASX_DGemm: Tin irrelevant, BLASX works with set BLOCKSIZE_DGEMM");
		...
		sprintf(filename, "%s/Data_manipulation/Results/%s/validation/BLASX_Dgemm_%d.log", PROJECTDIR, MACHINE, dev_id);	
	}
	else sprintf(filename, "%s/Data_manipulation/Results/%s/evaluation/BLASX_Dgemm_%d.log", PROJECTDIR, MACHINE, dev_id);	

	check_log(filename, M, N, K, BLOCKSIZE_DGEMM, A_loc, B_loc, C_loc, dev_id);
	...
	cudaCheckErrors();
	cpu_timer  = csecond() - cpu_timer;
	fprintf(stderr,"First BLASXt DGEMM call-> M = %zu, N = %zu, K = %zu\n", M, N, K);
	...
	fprintf(stderr,"Running BLASXt DGEMM-> M = %zu, N = %zu, K = %zu\n", M, N, K);
...
#endif
	report_results(filename, M, N, K, BLOCKSIZE_DGEMM, A_loc, B_loc, C_loc, dev_id, avg_t, min_t, max_t, first_over_t); 
	cudaCheckErrors();
