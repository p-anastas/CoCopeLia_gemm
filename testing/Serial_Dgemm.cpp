...
int main(const int argc, const char *argv[]) {
	...
	sprintf(filename, "%s/Data_manipulation/Results/%s/evaluation/Serial_Dgemm_%d.log", PROJECTDIR, MACHINE, dev_id);
	...
	double *A_dev, *B_dev, *C_dev; 
 	if (A_loc) A_dev = (double*) CoComalloc(M * K*sizeof(double), 0, dev_id);
	else A_dev = A; 
 	if (B_loc) B_dev = (double*) CoComalloc(N * K*sizeof(double), 0, dev_id);
	else B_dev = B; 
 	if (C_loc) C_dev = (double*) CoComalloc(M * N*sizeof(double), 0, dev_id);
	else C_dev = C; 
	cublasHandle_t handle;
	massert(CUBLAS_STATUS_SUCCESS == cublasCreate(&handle), "Serial_Dgemm: NOT CUBLAS_STATUS_SUCCESS");
	cudaCheckErrors();
	cpu_timer  = csecond() - cpu_timer ;
	...
	double send_t = csecond();
 	if (A_loc) vec_set_memcpy(A_dev, A,  M * K *sizeof(double), dev_id);
 	if (B_loc) vec_set_memcpy(B_dev, B,  N * K *sizeof(double), dev_id);
 	if (C_loc) vec_set_memcpy(C_dev, C,  M * N *sizeof(double), dev_id);
	cudaCheckErrors();
	...
	cpu_timer = csecond();
	massert(CUBLAS_STATUS_SUCCESS == cublasDgemm(handle, gpu_op_A, gpu_op_B, M, N, K, &alpha, A_dev, ldA, B_dev, ldB, &beta, C_dev, ldC), "Serial_Dgemm: NOT CUBLAS_STATUS_SUCCESS");
	cudaCheckErrors();
	cpu_timer  = csecond() - cpu_timer;
	fprintf(stderr,"First Serial DGEMM call-> M = %zu, N = %zu, K = %zu\n", M, N, K);
	...
	double get_t = csecond();
 	if (C_loc) vec_get_memcpy(C, C_dev,  M * N *sizeof(double), dev_id);
	cudaCheckErrors();
	get_t = csecond() - get_t; 
	fprintf(stderr, "done.\nD2H time:\t%lf ms\n\n",  get_t  * 1000);
	...
	fprintf(stderr,"Running Serial DGEMM-> M = %zu, N = %zu, K = %zu\n", M, N, K);
	...
	cublasXt_dgemm_wrapper(gpu_op_A,  gpu_op_B, M, N, K, alpha, A, ldA, B, ldB, beta, C, ldC,  (size_t) fmin(fmin(M,N),K)/2, 0, dev_id);
 	if (C_loc) optimized_memcpy(C_out1, C,  M * N *sizeof(double));
	else vec_get_memcpy(C_out1, C,  M * N *sizeof(double), dev_id);
