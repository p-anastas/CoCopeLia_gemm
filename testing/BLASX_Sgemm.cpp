...
int main(const int argc, const char *argv[]) {
	...
	if (Tin > 0) {
		if (Tin != BLOCKSIZE_SGEMM) warning("BLASX_SGemm: Tin irrelevant, BLASX works with set BLOCKSIZE_SGEMM");
		...
		sprintf(filename, "%s/Data_manipulation/Results/%s/validation/BLASX_Sgemm_%d.log", PROJECTDIR, MACHINE, dev_id);	
	}
	else sprintf(filename, "%s/Data_manipulation/Results/%s/evaluation/BLASX_Sgemm_%d.log", PROJECTDIR, MACHINE, dev_id);	

	check_log(filename, M, N, K, BLOCKSIZE_SGEMM, A_loc, B_loc, C_loc, dev_id);
	...
	cudaCheckErrors();
	cpu_timer  = csecond() - cpu_timer;
	fprintf(stderr,"First BLASXt SGEMM call-> M = %zu, N = %zu, K = %zu\n", M, N, K);
	...
	fprintf(stderr,"Running BLASXt SGEMM-> M = %zu, N = %zu, K = %zu\n", M, N, K);
...
#endif
	report_results(filename, M, N, K, BLOCKSIZE_SGEMM, A_loc, B_loc, C_loc, dev_id, avg_t, min_t, max_t, first_over_t); 
	cudaCheckErrors();
