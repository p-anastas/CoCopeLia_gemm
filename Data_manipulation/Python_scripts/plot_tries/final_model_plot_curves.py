...
for func in funcs:
	for machine in machine_names:
		...
		streamed_list = list(validata_plot[(validata_plot['T'] >= mindim) & (validata_plot['T'] <= maxdim)]['CoCopeLia_t'])
		streamed_list_flops = list(map(lambda x: GigaVal_per_s(dgemm_flops(dim[0],dim[1],dim[2]),x*1024),streamed_list))
		...
		werk_list = list(pred_plot_avg[(pred_plot_avg['T'] >= mindim) & (pred_plot_avg['T'] <= maxdim)]['werkhoven'])
		werk_list_flops = list(map(lambda x: GigaVal_per_s(dgemm_flops(dim[0],dim[1],dim[2]),x*1024),werk_list))
		CoCo_list = list(pred_plot_avg[(pred_plot_avg['T'] >= mindim) & (pred_plot_avg['T'] <= maxdim)]['CoCopelia'])
		CoCo_list_flops = list(map(lambda x: GigaVal_per_s(dgemm_flops(dim[0],dim[1],dim[2]),x*1024),CoCo_list))

		pred_tile_points = list(pred_plot_avg[(pred_plot_avg['T'] >= mindim) & (pred_plot_avg['T'] <= maxdim)]['T'])
		...
		print(werk_list)
		print(CoCo_list)
		plt.plot(pred_tile_points, werk_list_flops,
				 linewidth=1, color=color_werk, label='Werk.')
		plt.plot(val_tile_points, streamed_list_flops,
				 '.', markersize=2, color=color_mine, label='CoCoPeLia')
		plt.plot(pred_tile_points, CoCo_list_flops,
				 linewidth=1, color=color_mine, label='Eq. 12')
		...
		ax.fill_between(val_tile_points, CoCo_list_flops_min, CoCo_list_flops_max, alpha=0.2)
		...
		ax.set_ylabel('Flops (Tflops/s)')
		ax.set_xlabel('Tile Dim (T)')

		ax.set_ylim(1,10)

		#fig.set_size_inches(width, height)
		fig.savefig('final_model_curves_dim-%d-%d-%d_loc-%d-%d-%d.png' % (dim[0], dim[1], dim[2], loc[0], loc[1], loc[2]))
