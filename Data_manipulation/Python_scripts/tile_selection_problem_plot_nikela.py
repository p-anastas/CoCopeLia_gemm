	...
	for machine in machine_names:
		...
		for ctr in range(n):
			if (machine == 'testbed-I_Tesla-K40'):
				plt.text(maxT[ctr]+ 120, y_bot*1.3, str(maxT[ctr]), rotation=90, fontsize=font, color=mplt.colors2_2(ctr)) # 0.42	
			else:
				plt.text(maxT[ctr]+ 120, y_bot*2.1, str(maxT[ctr]), rotation=90, fontsize=font, color=mplt.colors2_2(ctr)) # 0.42	
		...
		if (machine == 'testbed-I_Tesla-K40'):
			...
			ax.set_ylim(1,6);
		#ax.set_ylim(1,10)
		ax.set_xlim(999,10500)
		fig.set_size_inches(width, height)
		if machine=='testbed-I_Tesla-K40':
			fig.subplots_adjust(left=.15, bottom=.18, right=.99, top=.80)
			fig.legend(by_label.values(), by_label.keys(),
				loc="upper left",  bbox_to_anchor=(0.18, 1), # Position of legend
