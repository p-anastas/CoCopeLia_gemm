...
labels = ['previous', '\(Ov_{pot}\)', 'CoCoPeLia']
...
achieved_mine = [[1.039741e+00,	7.442174e+00,						7.536597e+00,	4.410322e-01, 	1.447691e+00]] 				# 3.389425039,
...
achieved_mine.append([2.012398e-01, 						1.251084e+00, 	1.273289e+00,							9.937716e-02, 						4.146810e-01]) 				#7.097652e-01,
...
for m in range(0,machine_num):
	...
	for n in range(0,cases):
		...
		achieved_final.append(1 + (zero_over - achieved_mine[m][n])/zero_over - achieved_normalized[n])
		ker_pot.append( 1 + (zero_over - full_over)/zero_over - achieved_normalized[n] - achieved_final[n])
	if m == 0:
		plt.bar(ind + m*slot_ind, achieved_normalized, bar_width, label = labels[0], color=color_cublasxt)
		plt.bar(ind + m*slot_ind, achieved_final, bar_width, bottom = achieved_normalized, label = labels[2], color=color_mine)
		bot = list(map(lambda x,y: x +y, achieved_normalized, achieved_final))
		plt.bar(ind + m*slot_ind, ker_pot, bar_width, bottom = bot, label = labels[1], color=color_ideal)
	else:
		plt.bar(ind + m*slot_ind, achieved_normalized, bar_width, color=color_cublasxt)
		plt.bar(ind + m*slot_ind, achieved_final, bar_width, bottom = achieved_normalized, color=color_mine)
		bot = list(map(lambda x,y: x +y, achieved_normalized, achieved_final))
		plt.bar(ind + m*slot_ind, ker_pot, bar_width, bottom = bot, color=color_ideal)
...
#autolabel(rect)
#plt.xlabel('Problem')
plt.xticks(ind, case_names, fontsize=font)# ,rotation=20)
...
fig.savefig('motivation_plox_finale.pdf')
