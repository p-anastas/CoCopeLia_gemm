...
void CoCopeLia_Sgemm(CBLAS_TRANSPOSE cpu_op_A,  CBLAS_TRANSPOSE cpu_op_B, size_t M, size_t N, size_t K, float alpha, float* A, size_t ldA, float* B, size_t ldB, float beta, float* C, size_t ldC, int* Tin, short dev_id){
	...
	if ( cur_id != dev_id) printf("CoCo_Sgemm: Device change initiated(%d->%d)\n",cur_id, dev_id);
...
for (int mi = 0; mi < M_parts; mi++)
  	for (int ni = 0 ; ni < N_parts; ni++)
	{
		for (int ki = 0; ki < K_parts; ki++)
		{
			...
			if (!ki) CoCopeLia_Sgemm_subkernel_async(alpha, beta, kernels[current_ctr],d2hWaitForH2d);
        		else CoCopeLia_Sgemm_subkernel_async(alpha, 1.0, kernels[current_ctr], d2hWaitForH2d);
    		}
	}
if (C_loc){
	for (int mi = 0; mi < M_parts; mi++)
		for (int ni = 0 ; ni < N_parts; ni++)
		{
        		...
        		CoCopeLia_Sgemm_subkernel_out(kernels[current_ctr]);
    		}
}
cudaCheckErrors();
cpu_timer = csecond() - cpu_timer;
