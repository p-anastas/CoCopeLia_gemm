...
int main(const int argc, const char *argv[]) {
	...
	sprintf(filename, "%s/BenchOutputs/%s/cublasDgemm_dev-%d_min-%d_max-%d_step-%d_iter-%d.log", PROJECTDIR, MACHINE, dev_id, minDim, maxDim, step, ITER);
	...
	double *A_dev, *B_dev, *C_dev;
  	...
  	vec_alloc((void**)&B_dev, maxDim * maxDim * sizeof(double), dev_id);
  	vec_alloc((void**)&C_dev, maxDim * maxDim * sizeof(double), dev_id);
	cudaCheckErrors();
	...
	Dvec_init_cuRAND(B_dev, maxDim * maxDim, 43);
	Dvec_init_cuRAND(C_dev, maxDim * maxDim, 44);
